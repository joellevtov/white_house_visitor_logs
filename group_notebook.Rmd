...
```{r}
wh_visitor_data <- wh_visitor_data |>
  select(-uin, -bdgnbr, -access_type, -appt_cancel_date, -last_updatedby, -post, -lastentrydate, -terminal_suffix, -caller_name_last, -caller_name_first, -caller_room, -column, -column2)
```

Question 1.  What are the most popular months and dates? What political events correlate with those?

WHERE WE ARE AT RIGHT NOW: 
Menna, can you run this code again and see if you get the same results? I realized our appt end date column had some things listed as 1/1/2021 11:59:00 and some as 23:59:00. I think we should get the same answers but want to make sure it's uniform. I wrote some code in the chunk below to clean that column so it's just date no time.
