...
# If this code doesn't work right run the table code first, then run it with the wh_repeats <- in front of it. That'll do the trick. This shows us how many repeats of certain numbers of visitors there are.
...
# So the vast majority of repeated numbers are low. Let's take a closer look.
...
# So days with visitors roundabout 500 are the norm.
```
...
Could be worth looking into. His top visitor only came 4 times - I think we'd need to know more about his role to know why many people are coming to visit him a few times.

Peyton Schwartz 4533 - Special Assistant to the Social Secretary & Deputy Director of Arrangements
Could be worth looking into - looks like she's also getting lots of people visiting a few times. Same feedback as for Ballen.
...
wh_visitor_data |> 
 filter(visitee_namelast_clean == "Ballen" & visitee_namefirst_clean == "Kevin") |>
  group_by(namelast, namefirst, namemid) |>
  summarise(total_visits = n()) |>
  arrange(desc(total_visits))

wh_visitor_data |> 
 filter(visitee_namelast_clean == "Schwartz" & visitee_namefirst_clean == "Peyton") |>
  group_by(namelast, namefirst, namemid) |>
  summarise(total_visits = n()) |>
  arrange(desc(total_visits))
...
WHERE WE ARE AT: 
Derek had some feedback about looking into more AI ppl / foreign ambassadors. Shaun I think you were working on this? We should tag team this probably - you want to take AI folks and someone else can look into foreign ambassadors or something?
